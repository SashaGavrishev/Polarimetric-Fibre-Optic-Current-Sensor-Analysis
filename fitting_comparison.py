...
def import_data(path):
    ...
    scope_time_interval = extract_number(
        read_file_details(path, 10, 8)[1]
    )  # uS
...
data = filter_data(data, 8, 8, 8, 981, 1181, True)
...
fig2, ax2, line2_1 = lissajous_plot(data["CH3e"], data["CH4e"])

ax2.plot(data["CH3f"], data["CH4f"], color="darkred", alpha=0.5)
...
geoCov = cov_geo_params(
    np.array([theta_fgee]).T, data["CH3e"], data["CH4e"]
)
